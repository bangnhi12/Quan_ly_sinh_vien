...
def assign_mon(ma_nganh):
    ...
    # Lấy các môn đã gán cho ngành này
...
def report():
    ...
    # Convert sang list với tổng cộng
    khoas_grouped_stats = []
    total_mons_all = 0
    total_credits_all = 0
    
    for khoa_code in sorted(khoas_stats.keys()):
        khoa_nganhs_stats = khoas_stats[khoa_code]
        
        # Tính tổng cho khoa này
        khoa_total_mons = sum(s['mon_count'] for s in khoa_nganhs_stats)
        khoa_total_credits = sum(s['total_tin_chi'] for s in khoa_nganhs_stats)
        
        # Cộng vào tổng chung
        total_mons_all += khoa_total_mons
        total_credits_all += khoa_total_credits
        
        khoas_grouped_stats.append({
            'khoa': khoa_nganhs_stats[0]['khoa'],
            'stats': khoa_nganhs_stats,
            'khoa_total_mons': khoa_total_mons,
            'khoa_total_credits': khoa_total_credits
        })
    
    return render_template('admin/ganmon_report.html', 
                         khoas_grouped_stats=khoas_grouped_stats,
                         total_mons_all=total_mons_all,
